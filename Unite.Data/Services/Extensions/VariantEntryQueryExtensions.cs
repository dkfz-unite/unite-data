...
public static class VariantEntryQueryExtensions
{
    ...
    /// <typeparam name="TVE">Variant entry type.</typeparam>
    /// <returns>Query with variants filtered by specimen ids.</returns>
    public static IQueryable<TVE> FilterBySpecimenIds<TVE>(this IQueryable<TVE> query, IEnumerable<int> specimenIds)
        where TVE : VariantEntry<Variant>
    {
        ...
        throw new ArgumentException($"Unsupported variant entry type: {typeof(TVE).Name}");
    }
    ...
    public static IQueryable<TVE> FilterByRange<TVE>(this IQueryable<TVE> query, Chromosome chromosomeId, int start, int end)
        where TVE : VariantEntry<Variant>
    {
        if (typeof(TVE) == typeof(SSM.VariantEntry))
        {
